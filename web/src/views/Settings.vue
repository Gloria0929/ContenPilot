<template>
  <div class="settings">
    ...
    <el-card shadow="never" class="apikey-card">
      <template #header>
        <div class="apikey-header">
          <span>API 密钥</span>
          <el-button type="primary" @click="showCreate = true">生成密钥</el-button>
        </div>
      </template>
      ...
      <el-table :data="apiKeys" row-key="id">
        ...
        <el-table-column label="Access Key" min-width="160">
          <template #default="scope">
            <span v-if="scope.row.access_key" class="ak-text">{{ scope.row.access_key }}</span>
            <span v-else class="ak-legacy">旧格式密钥</span>
          </template>
        </el-table-column>
        <el-table-column label="跳过审核权限" min-width="110">
          <template #default="scope">
            <el-tag size="small" :type="scope.row.allow_override_review ? 'warning' : 'info'">
              {{ scope.row.allow_override_review ? "允许" : "不允许" }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="最近使用" min-width="150">
          <template #default="scope">{{ fmtTime(scope.row.last_used_at) }}</template>
        </el-table-column>
    ...
    <el-dialog v-model="showCreate" title="生成 API 密钥" width="460px">
      <el-form label-position="top" @submit.prevent="createKey">
        <el-form-item label="名称">
          <el-input v-model="createForm.name" placeholder="用途标识，如 ai-skill" />
        </el-form-item>
    ...
    <el-dialog v-model="showResult" title="密钥已生成" width="560px" :close-on-click-modal="false">
      <el-alert
        type="warning"
        :closable="false"
        show-icon
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="配置文件（保存到 ~/.contentpilot/api_client.json，供 AI Skill 调用 API）">
          <div class="key-row">
            <el-input :model-value="configSnippet" type="textarea" :rows="5" readonly />
            <el-button @click="copy(configSnippet)">复制</el-button>
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button type="primary" @click="showResult = false">我已保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
async function createKey() {
  ...
  try {
    ...
    created.value = { access_key: r.data.access_key, secret_key: r.data.secret_key };
